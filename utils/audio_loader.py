...
class DataHandler:
    ...
    def _load_data(self, config):
        ...
        if os.path.exists(self.audio_path):
            self.processed_audio = pickle.load(open(self.audio_path, 'rb'), encoding='latin1')
        else:
            self.processed_audio = self._process_audio(self.raw_audio)
            if self.audio_path != '':
                pickle.dump(self.processed_audio, open(self.audio_path, 'wb'))
